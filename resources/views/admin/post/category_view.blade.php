...
@section('title')
<title>{{$category}} | news 24/7</title>
@endsection
@section('contents')
        <!-- ============================================================== -->
        <!-- Page wrapper  -->
            ...
            <div class="container-fluid">
                ...
                <div class="card card-default">
                    ...
                    <div class="card-body collapse show">
                        <div class="" style="width:100%;overflow-x:scroll;">
                            <table id="category_active" class="table text-center">
                                <thead>
                                    <tr>
                                        <th>Serial</th>
                                        <th>Headline</th>
                                ...
                                <tbody>
                                    @foreach ($select as $select)
                                        <tr>
                                            ...
                                            <td>{{$i++}}</td>
                                            <td>{{Str::limit($select->heading,$limit=50,$end=" ...")}}</td>
                                            <td>{{$select->category}}</td>
                                            <td>{{$select->total_view}}</td>
                                            <td>{{Str::limit($select->tag,$limit=30,$end=" ...")}}</td>
                                            ...
                                            <td>
                                                <label class="button">
                                                    @php $j=$select->add_to_latest_news; if($j==1) $url1='data-url';else $url1='';@endphp
                                                    @php $j=$select->add_to_latest_news; if($j==0) $url2='data-url';else $url2='';@endphp
                                                    <input type="checkbox" id="{{$select->slug}}" {{$url1}}="{{route("remove_from_latest_news",$select->slug)}}" {{$url2}}="{{route("add_to_latest_news",$select->slug)}}">
                ...
                <div class="card card-default">
                    <div class="card-header">
                        ...
                        <h4 class="card-title m-b-0">All deactive POSTS</h4>
                    </div>
                    <div class="card-body collapse show">
                        <div class="" style="width:100%;overflow-x:scroll;">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <label class="button">
                                                    @php $j=$select->add_to_latest_news; if($j==1) $url1='data-url';else $url1='';@endphp
                                                    @php $j=$select->add_to_latest_news; if($j==0) $url2='data-url';else $url2='';@endphp
                                                    <input type="checkbox" id="{{$select->id}}" {{$url1}}="{{route("remove_from_latest_news",$select->slug)}}" {{$url2}}="{{route("add_to_latest_news",$select->slug)}}">
                                                    <span @php if($j==0) echo "class='deactive'"; @endphp></span>
                                                    <span></span>
                                                    <span></span>
                                                </label>
                                            ...
                                            <td>
                                                ...
                                                <a href="{{route('post_restore',$select->slug)}}" onclick="return confirm('restore Post!!')" class="text-inverse" title="" data-toggle="tooltip" data-original-title="post restore">
                                                    <i class="mdi mdi-backup-restore"></i>
                                                </a>
                                                <a href="{{route("post_delete",$select->slug)}}" onclick="return confirm("delete Post!!")" class="text-inverse" title="" data-toggle="tooltip" data-original-title="delete">
                                                    <i class="ti-trash"></i>
                                                </a>
                                            </td>
                                        </tr>
